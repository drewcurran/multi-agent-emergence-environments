                    ...
                    if layer_type == 'dense':
                        ...
                        with tf.compat.v1.variable_scope(layer_name):
                            if 'mask' in layer:
                                ...
                                if pool_type == 'avg_pooling':
                                    inp[nodes_out[0]] = tf.reduce_mean(input_tensor=inp[nodes_in[0]], axis=-2)
                                elif pool_type == 'max_pooling':
                                    inp[nodes_out[0]] = tf.reduce_max(input_tensor=inp[nodes_in[0]], axis=-2)
                        ...
                        with tf.compat.v1.variable_scope(layer_name, reuse=reuse):
                            inp[nodes_out[0]] = slim.layer_norm(inp[nodes_in[0]], begin_norm_axis=2)
                    else:
