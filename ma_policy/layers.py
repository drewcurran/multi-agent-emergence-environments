...
def entity_avg_pooling_masked(x, mask):
    ...
    masked = x * mask
    summed = tf.reduce_sum(input_tensor=masked, axis=-2)
    denom = tf.reduce_sum(input_tensor=mask, axis=-2) + 1e-5
    return summed / denom
...
def entity_max_pooling_masked(x, mask):
    ...
    '''
    mask = tf.expand_dims(mask, -1)
    has_unmasked_entities = tf.sign(tf.reduce_sum(input_tensor=mask, axis=-2, keepdims=True))
    offset = (mask - 1) * 1e9
    masked = (x + offset) * has_unmasked_entities
    return tf.reduce_max(input_tensor=masked, axis=-2)
...
def residual_sa_block(inp, mask, heads, n_embd,
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        post_a_mlp = tf.compat.v1.layers.dense(a,
                                     n_embd,
                                     kernel_initializer=tf.compat.v1.random_normal_initializer(stddev=post_scale),
                                     name="mlp1")
        x = inp + post_a_mlp
        ...
        if n_mlp > 1:
            ...
            mlp = tf.compat.v1.layers.dense(mlp,
                                  n_embd,
                                  kernel_initializer=tf.compat.v1.random_normal_initializer(stddev=mlp2_scale),
                                  name="mlp2")
        if n_mlp > 2:
            mlp3_scale = np.sqrt(mlp_w2 / n_embd)
            mlp = tf.compat.v1.layers.dense(mlp,
                                  n_embd,
                                  kernel_initializer=tf.compat.v1.random_normal_initializer(stddev=mlp3_scale),
                                  name="mlp3")
        if n_mlp > 1:
            x = x + mlp
        return x
...
def self_attention(inp, mask, heads, n_embd, layer_norm=False, qk_w=1.0, v_w=0.01,
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        with tf.compat.v1.variable_scope('flatten_heads'):
            out = tf.transpose(a=att_sum, perm=(0, 1, 3, 2, 4))  # (bs, T, n_output_entities, heads, features)
...
def stable_masked_softmax(logits, mask):
    ...
    with tf.compat.v1.variable_scope('stable_softmax'):
        ...
        logits -= tf.reduce_max(input_tensor=logits, axis=-1, keepdims=True)
        unnormalized_p = tf.exp(logits)

        #  Mask the unnormalized probibilities and then normalize and remask
        if mask is not None:
            unnormalized_p *= mask
        normalized_p = unnormalized_p / (tf.reduce_sum(input_tensor=unnormalized_p, axis=-1, keepdims=True) + 1e-10)
        if mask is not None:
            normalized_p *= mask
    return normalized_p
...
def qkv_embed(inp, heads, n_embd, layer_norm=False, qk_w=1.0, v_w=0.01, reuse=False):
    ...
    with tf.compat.v1.variable_scope('qkv_embed'):
        ...
        qk = tf.compat.v1.layers.dense(inp,
                             n_embd * 2,
                             kernel_initializer=tf.compat.v1.random_normal_initializer(stddev=qk_scale),
                             reuse=reuse,
        ...
        value = tf.compat.v1.layers.dense(inp,
                                n_embd,
                                kernel_initializer=tf.compat.v1.random_normal_initializer(stddev=v_scale),
                                reuse=reuse,
                                name="v_embed")  # bs x T x n_embd
        value = tf.reshape(value, (bs, T, NE, heads, n_embd // heads))

        query = tf.transpose(a=query, perm=(0, 1, 3, 2, 4),
                             name="transpose_query")  # (bs, T, heads, NE, n_embd / heads)
        key = tf.transpose(a=key, perm=(0, 1, 3, 4, 2),
                           name="transpose_key")  # (bs, T, heads, n_embd / heads, NE)
        value = tf.transpose(a=value, perm=(0, 1, 3, 2, 4),
...
def layernorm(x, scope, epsilon=1e-5, reuse=False):
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        gain = tf.compat.v1.get_variable('gain', [n_state], initializer=tf.compat.v1.constant_initializer(1))
        bias = tf.compat.v1.get_variable('bias', [n_state], initializer=tf.compat.v1.constant_initializer(0))
